<?php

declare(strict_types=1);

namespace Tests\Browser;

use App\Models\BetaAccessKey;
use App\Models\Organisation;
use Laravel\Dusk\Browser;
use Tests\DuskTestCase;

final class RegistrationWithBetaKeyTest extends DuskTestCase
{
    public function test_user_can_register_with_valid_beta_key(): void
    {
        $organisation = Organisation::factory()->create();
        $betaKey = BetaAccessKey::factory()->create([
            'organisation_id' => $organisation->id,
        ]);

        $this->browse(function (Browser $browser) use ($betaKey) {
            $browser->visit(route('register'))
                ->waitFor('#register-form')
                ->click('#reg-path-key')
                ->waitFor('#reg-beta-key')
                ->type('[wire\\:model="name"]', 'Beta User')
                ->type('[wire\\:model="email"]', 'beta-user@example.com')
                ->type('[wire\\:model="password"]', 'password')
                ->type('[wire\\:model="password_confirmation"]', 'password')
                ->type('#reg-beta-key', $betaKey->key)
                ->click('#register-submit')
                ->waitForLocation('/')
                ->assertSee('Online Waschnusswetten');
        });

        $this->assertDatabaseHas('users', [
            'email' => 'beta-user@example.com',
        ]);
    }
}
