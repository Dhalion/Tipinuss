        ...
        <form wire:submit="register" class="space-y-5">
            ...
            @if ((bool) config('app.beta_mode', false))
                <div x-data="{ hasBetaKey: $wire.entangle('hasBetaKey') }" class="space-y-3">
                    ...
                    <label
                        :class="hasBetaKey ? 'border-primary-400 bg-primary-50/50 dark:bg-primary-900/20 dark:border-primary-600' : 'border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800'"
                    ...
                    <div x-show="!hasBetaKey" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" class="flex items-start gap-2 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50 px-3 py-2.5">
