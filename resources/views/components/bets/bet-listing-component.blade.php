@php
    ...
    $cardClasses = 'space-y-4 transition-all duration-200';
    if (!$bet->isClosed()) {
        $cardClasses .= ' hover:shadow-md hover:border-primary-500/50';
    }
@endphp
@if($bet->isClosed())
    <a wire:key="bet-card-{{ $bet->id }}" href="{{ route('bets.detail', ['bet' => $bet->slugUrl()]) }}" wire:navigate.hover class="block opacity-60">
@else
    <a wire:key="bet-card-{{ $bet->id }}" href="{{ route('bets.detail', ['bet' => $bet->slugUrl()]) }}" wire:navigate.hover class="block">
@endif
    <flux:card class="{{ $cardClasses }}">
        ...
        <div class="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">
            ...
            @foreach($bet->betOptions as $option)
                <div wire:key="bet-{{ $bet->id }}-opt-{{ $option->id }}" class="rounded-lg bg-zinc-100 dark:bg-zinc-700 px-3 py-2.5 text-center">
